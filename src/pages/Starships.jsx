...
const Starships = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const data = await getStarships(page, searchTerm);
        setStarships(data.results);
        setTotalPages(Math.ceil(data.count / 10));
      } catch {
        setError('An error occurred while loading starships. Please try again.');
      } finally {
        setLoading(false);
      }
    };
    ...
    >
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <h1 className="text-4xl font-bold flex items-center gap-3">
          <Rocket className="text-red-500" size={36} /> Starships
        </h1>
      ...
      {loading ? (
        ...
        <p className="text-center text-gray-500 text-xl py-10">No starships found matching your criteria.</p>
      ) : (
          ...
          {starships.map((ship, index) => {
            ...
            return (
              <motion.div
                ...
                key={id}
              >
                <Link to={`/starships/${id}`} className="block h-full bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md hover:shadow-xl hover:-translate-y-1 transition-all border dark:border-gray-700">
                  <Rocket className="text-red-500 mb-4" size={32} />
                  <h2 className="text-xl font-bold mb-2 truncate text-gray-900 dark:text-white">{ship.name}</h2>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Model: <span className="text-gray-700 dark:text-gray-300 truncate block">{ship.model}</span></p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Manufacturer: <span className="text-gray-700 dark:text-gray-300 truncate block">{ship.manufacturer}</span></p>
                </Link>
              </motion.div>
            );
          })}
