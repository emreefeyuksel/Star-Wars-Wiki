...
const Species = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const data = await getSpecies(page, searchTerm);
        setSpecies(data.results);
        setTotalPages(Math.ceil(data.count / 10));
      } catch {
        setError('An error occurred while loading species. Please try again.');
      } finally {
        setLoading(false);
      }
    };
    ...
    >
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <h1 className="text-4xl font-bold flex items-center gap-3 text-gray-900 dark:text-white">
          <Dna className="text-teal-500" size={36} /> Species
        </h1>
      ...
      {loading ? (
        ...
        <p className="text-center text-gray-500 text-xl py-10">No species found matching your criteria.</p>
      ) : (
          ...
          {species.map((item, index) => {
            ...
            return (
              <motion.div
                ...
                key={id}
              >
                <Link to={`/species/${id}`} className="block h-full bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md hover:shadow-xl hover:-translate-y-1 transition-all border border-gray-200 dark:border-gray-700">
                  <Dna className="text-teal-500 mb-4" size={32} />
                  <h2 className="text-xl font-bold mb-2 truncate text-gray-900 dark:text-white">{item.name}</h2>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Classification: <span className="text-gray-700 dark:text-gray-300 truncate block capitalize">{item.classification}</span></p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Designation: <span className="text-gray-700 dark:text-gray-300 inline-block align-bottom capitalize">{item.designation}</span></p>
                </Link>
              </motion.div>
            );
          })}
